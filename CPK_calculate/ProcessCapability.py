...
class ProcessCapability:
    ...
    def calculate_cpk(self, USL=None, LSL=None, method='s/c4', num_groups=1):
        ...
        if method == 'r/d2' or group_size == 1:
            ...
            c4 = (4 * group_size - 4) / (4 * group_size - 3) if group_size != 1 else  1# c4 修正因子
            group_std_devs = [g.std(ddof=1) / c4 for g in grouped_data]
            avg_std_dev = np.mean(group_std_devs)
        elif method == 'merged':
            total_variance = sum([(len(g) - 1) * (g.std(ddof=1) ** 2) for g in grouped_data])
            total_samples = sum([len(g) for g in grouped_data])
            avg_std_dev = np.sqrt(total_variance / (total_samples - num_groups))
        else:
            raise ValueError("method 必须是 'r/d2' 'merged',或 's/c4'")
...
22.9
    ...
    cpk_value_r_d2 = process_capability.calculate_cpk(USL, LSL, method='merged')
